...
function OnFinish(selProj, selObj)
{
	try {
		...
		AddCommonConfig(selProj, strProjectName, false);
		AddCustomConfig(selProj, strProjectName);
		AddFilters(selProj);
		...
		// copy WiEngine libs and headers
		var fso = new ActiveXObject("Scripting.FileSystemObject");
		var strTemplatePath = wizard.FindSymbol("TEMPLATES_PATH");
		fso.CopyFolder(strTemplatePath + "\\WiEngine", strProjectPath + "\\WiEngine");
...
function AddFilters(proj)
{
	...
	{
		// 将文件夹添加到项目
		var group = proj.Object.AddFilter('src');
		group.Filter = 'cpp';
...
function AddCustomConfig(proj, strProjectName)
{
    try {
        // component options
		...
		CLTool.PreprocessorDefinitions = "WIN32;WINDOWS;" + CLTool.PreprocessorDefinitions;

		// disable warnings
		CLTool.DisableSpecificWarnings = "4819;4244";
		...
		var includes = "WiEngine\\headers; \
            ...
            WiEngine\\headers\\WiEngine\\include\\utils; \
            WiEngine\\headers\\WiEngine\\include\\types; \
            WiEngine\\headers\\WiEngine\\include\\transitions; \
            WiEngine\\headers\\WiEngine\\include\\tmx; \
            WiEngine\\headers\\WiEngine\\include\\platform\\windows; \
            WiEngine\\headers\\WiEngine\\include\\persistence; \
            WiEngine\\headers\\WiEngine\\include\\particle; \
            WiEngine\\headers\\WiEngine\\include\\opengl; \
            WiEngine\\headers\\WiEngine\\include\\nodes; \
            WiEngine\\headers\\WiEngine\\include\\mfs; \
            WiEngine\\headers\\WiEngine\\include\\lua; \
            WiEngine\\headers\\WiEngine\\include\\json; \
            WiEngine\\headers\\WiEngine\\include\\iap; \
            WiEngine\\headers\\WiEngine\\include\\gridactions; \
            WiEngine\\headers\\WiEngine\\include\\grid; \
            WiEngine\\headers\\WiEngine\\include\\filters; \
        ...
        // library folder
        LinkTool.AdditionalLibraryDirectories = "WiEngine\\libs;" + LinkTool.AdditionalLibraryDirectories;
        ...
        if (useWiNetwork)
            LinkTool.AdditionalDependencies = "libwinetwork.lib " + LinkTool.AdditionalDependencies;
        if (useLua)
