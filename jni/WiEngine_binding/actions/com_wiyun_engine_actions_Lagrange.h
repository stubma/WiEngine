/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_wiyun_engine_actions_Lagrange */

#ifndef _Included_com_wiyun_engine_actions_Lagrange
#define _Included_com_wiyun_engine_actions_Lagrange
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_wiyun_engine_actions_Lagrange
 * Method:    nativeInit
 * Signature: (FLcom/wiyun/engine/types/WYLagrangeConfig;)V
 */
JNIEXPORT void JNICALL Java_com_wiyun_engine_actions_Lagrange_nativeInit
  (JNIEnv *, jobject, jfloat, jobject);

/*
 * Class:     com_wiyun_engine_actions_Lagrange
 * Method:    setAutoRotate
 * Signature: (ZF)V
 */
JNIEXPORT void JNICALL Java_com_wiyun_engine_actions_Lagrange_setAutoRotate
  (JNIEnv *, jobject, jboolean, jfloat);

/*
 * Class:     com_wiyun_engine_actions_Lagrange
 * Method:    setPinPoint
 * Signature: (FF)V
 */
JNIEXPORT void JNICALL Java_com_wiyun_engine_actions_Lagrange_setPinPoint
  (JNIEnv *, jobject, jfloat, jfloat);

/*
 * Class:     com_wiyun_engine_actions_Lagrange
 * Method:    nativeGetPinPoint
 * Signature: (Lcom/wiyun/engine/types/WYPoint;)V
 */
JNIEXPORT void JNICALL Java_com_wiyun_engine_actions_Lagrange_nativeGetPinPoint
  (JNIEnv *, jobject, jobject);

/*
 * Class:     com_wiyun_engine_actions_Lagrange
 * Method:    clearPinPoint
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_wiyun_engine_actions_Lagrange_clearPinPoint
  (JNIEnv *, jobject);

/*
 * Class:     com_wiyun_engine_actions_Lagrange
 * Method:    setPinAngleDelta
 * Signature: (F)V
 */
JNIEXPORT void JNICALL Java_com_wiyun_engine_actions_Lagrange_setPinAngleDelta
  (JNIEnv *, jobject, jfloat);

/*
 * Class:     com_wiyun_engine_actions_Lagrange
 * Method:    getPinAngleDelta
 * Signature: ()F
 */
JNIEXPORT jfloat JNICALL Java_com_wiyun_engine_actions_Lagrange_getPinAngleDelta
  (JNIEnv *, jobject);

#ifdef __cplusplus
}
#endif
#endif
