...
static const FontInitRec gDefaultSystemFonts[] = {
...
//    load_font_info_xml("/system/etc/system_fonts.xml");
    ...
    /*
     * if android doesn't have font config xml file, use default font list to make it work
     */
    if(gNumSystemFonts <= 0) {
    	for (size_t i = 0; i < SK_ARRAY_COUNT(gDefaultSystemFonts); i++) {
    		*fontInfo.append() = gDefaultSystemFonts[i];
    	}
    	gNumSystemFonts = fontInfo.count();
    }

    /*
     * if can't decide a default font names, use sans names
     */
    if(gDefaultNames == NULL)
    	gDefaultNames = (char**)gSansNames;
