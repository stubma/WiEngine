...
b2World::~b2World()
{
	...
	// delete java contact listener wrap
	if(m_jclWrap != NULL) {
		delete m_jclWrap;
		m_jclWrap = NULL;
	}

	// delete java destruction listener wrap
	if(m_jdlWrap != NULL) {
		delete m_jdlWrap;
		m_jdlWrap = NULL;
	}
...
void b2World::DestroyBody(b2Body* b)
{
    ...
    b2ControllerEdge* ctrle = b->m_controllerList;
    while(ctrle)
    {
		b2ControllerEdge* ctrle0 = ctrle;
		ctrle = ctrle->nextController;

		ctrle0->controller->RemoveBody(b);
    }
...
struct BOX2D_API b2WorldQueryWrapper
{
...
struct BOX2D_API b2WorldRayCastWrapper
{
	...
	case b2Shape::e_circle:
		{
			...
			if(fixture->IsSensor() || m_noSolidDraw)
	...
	case b2Shape::e_polygon:
		{
			...
			if(fixture->IsSensor() || m_noSolidDraw) {
				// to close polygon, LINE_LOOP is very slow in Desire HD so we have to use LINE_STRIP
				vertices[vertexCount] = b2Mul(xf, poly->m_vertices[0]);
				m_debugDraw->DrawPolygon(vertices, vertexCount + 1, color);
			} else {
