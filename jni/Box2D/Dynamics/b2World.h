...
public:
	...
	/// true means don't draw solid fixture
	void SetNoSolidDraw(bool flag);
	
	wyBox2D* GetBox2D() const;

	void SetBox2D(wyBox2D* box2d);
...
public:
	// for wrap java layer contact listener
	b2ContactListener* m_jclWrap;

	// for wrap java layer destruction listener
	b2DestructionListener* m_jdlWrap;
...
inline wyBox2D* b2World::GetBox2D() const
{
	return m_box2d;
}

inline void b2World::SetBox2D(wyBox2D* box2d)
{
	m_box2d = box2d;
}
...
inline void b2World::SetNoSolidDraw(bool flag) {
	m_noSolidDraw = flag;
}
